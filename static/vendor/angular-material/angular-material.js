...
function mdTextFloatDirective($mdTheming, $mdUtil, $parse) {
  return {
    ...
    compile : function(element, attr) {
      ...
      //pass along ng attributes to input element
      ...
      for (var key in attributes) {
        ...
        if(typeof(attribute.name) !== 'undefined') {
          ...
          if(name.substring(0,2) == 'ng' && name != 'ng-model') {
            attr.passAlong += name+'="'+attribute.value+'" ';
          }
        }
      ...
      return {
        pre : function(scope, element, attrs) {
          ...
          scope.passAlong = attr.passAlong;
        },
        post: $mdTheming
      };
    },
    ...
    ' <label for="{{fid}}" >{{label}}</label>' +
	  ' <md-input id="{{fid}}" ng-disabled="isDisabled()" ng-model="value" type="{{inputType}}" {{passAlong}} ></md-input>' +
    '</md-input-group>'
  };
}
